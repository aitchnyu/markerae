...
then
  docker run --mount type=bind,source="$(pwd)",target=/hostpwd -it markeraeprod venv/bin/python3 manage.py collectstatic --noinput && cp staticfiles /hostpwd/staticfiles
