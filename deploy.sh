...
then
  docker run --mount type=bind,source=.,target=/hostpwd -it markeraeprod /code/venv/bin/python3 manage.py collectstatic --noinput && cp staticfiles /hostpwd/staticfiles
